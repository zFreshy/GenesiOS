...
void gdt_init(void) {
    ...
    set_entry(&s_gdt.entries[2], 0, 0xFFFFF, 0x92, 0xA0); /* kernel data */
    set_entry(&s_gdt.entries[3], 0, 0xFFFFF, 0xFA, 0xA0); /* user code   */
    set_entry(&s_gdt.entries[4], 0, 0xFFFFF, 0xF2, 0xA0); /* user data   */
    set_entry(&s_gdt.entries[5], 0, 0,       0x00, 0x00); /* padding      */
