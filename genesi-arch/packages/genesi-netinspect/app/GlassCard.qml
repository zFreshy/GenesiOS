 ...
 *
 * Surface is the Genesi system blue (completing pkgrel 7's "match the rest of the
 * OS" intent — the card was the last green-tinted holdout). Set `accent` +
 * `active: true` to highlight with a coloured border; `interactive: false` on
 * purely decorative cards that shouldn't react.
 */
import QtQuick
